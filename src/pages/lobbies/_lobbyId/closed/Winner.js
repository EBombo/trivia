...
export const Winner = (props) => {
  ...
  useEffect(() => {
    ...
    const playAnimation = () => {
      ...
      _animate({
        ...
        onUpdate: (tween) => { updatePrizeIconCloned(tween); },
      })
      .then(() => _animate({
  ...
  useEffect(() => {
    ...
    const playAnimation = () => {
      ...
      const { width: firstPlaceNameNodeWidth, height: firstPlaceNameNodeHeight} = firstPlaceNameNode.getBoundingClientRect();
      ...
      delay(() => {
        _animate ({
          ...
          to: 0.5,
          duration: 1000,
          onPlay: () => { firstPlaceBgNode.style.opacity = 1; },
          onUpdate: (tween) => {
            const firstPlaceBgNode = firstPlaceBgRef.current;
  ...
  return (
    <WinnerCss award={award} isList={props.isList}>
      ...
      <div className={`${props.enableAnimation && "opacity-0 max-w-[10px]"} grid font-bold transition-all duration-500 ${props.award && "mt-[30px]"}`} ref={winnerLabelRef}>
        <div
          className={`my-auto ${props.isList ? "ml-0" : "ml-[-50px]"} text-3xl cursor-pointer flex h-[fit-content] rounded-lg text-black bg-white py-3 pl-14 pr-3 z-[1]`}
        ...
        {award && <div className={`bg-grayLighten rounded
          mt-[-10px] mb-auto ${props.isList ? "ml-0" : "ml-[-30px]" }
          pt-[24px] pr-[15px] pb-[15px] pl-[3rem]
          text-secondary text-2xl`}>{award}</div>}
      </div>

      { props.index === 0 && (
        <div className="absolute z-30 opacity-0 bg-white p-4 text-secondary text-2xl font-bold rounded-lg" ref={firstPlaceNameRef}>{props.winner.nickname}</div>
      )}
