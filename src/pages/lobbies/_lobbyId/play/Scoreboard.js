...
export const Scoreboard = (props) => {
  ...
  const RankingItem = (user, i) => (
    <div key={`rankint-item-${i}`}
      ...
      <div className={`px-4 self-center justify-self-start ${props.authUser?.id === user.id ? "text-success" : "text-whiteLight"}`}>Santiago</div>
      ...
      <div className="min-h-screen flex flex-col py-5 bg-opacity-50 px-4">
        ...
        {true && (
          <div className="mb-20 flex justify-end">
            <ButtonAnt color="success" width="200px" className="font-bold text-xl px-8">Siguiente</ButtonAnt>
          </div>
        )}

        <div className="mb-6">
          {ranking.map((_, i) => RankingItem(_, i))}
        </div>
        ...
        {true && (
          <div className="my-6 text-center flex justify-center">
            <ButtonAnt color="danger" width="200px" className="inline-block font-bold text-lg px-8">Finalizar</ButtonAnt>
          </div>
        )}
      </div>
    </div>
