  ...
  return (
    <div className="relative flex flex-col justify-center">
      <div className={`mx-5 py-5 relative`}>
        ...
        {props.isAnswered && (
          <Image className="absolute top-0 right-2" src={`${config.storageUrl}/resources/check_black.svg`} width="36px"/>
        )}
      </div>
      <div className="self-center">
        <ButtonAnt color="success" size="big" disabled={props.isAnswered}><span className="font-bold text-lg px-4">Enviar</span></ButtonAnt>
      </div>
    </div>
  );
};
